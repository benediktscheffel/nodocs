...
class OcrServiceImpl implements OcrService {
  ...
  Future<String> _uploadAsset(final pw.Document pdf, final String accessToken) async {
    String base = dotenv.env['BASE']==null ? '' : dotenv.env['BASE']!;
  ...
  Future<Uint8List> _pollDocument(final String location, final String accessToken, final Logger log) async {
    Map<String, dynamic> documentStatusAndDownloadUri = await _getStatusAndDownloadUri(location, accessToken);
    while (documentStatusAndDownloadUri['status'] == 'in progress') {
      log.i("Status: ${documentStatusAndDownloadUri['status']}");
      documentStatusAndDownloadUri = await _getStatusAndDownloadUri(location, accessToken);
    }
  ...
  Future<Map<String, dynamic>> _getStatusAndDownloadUri(final String location, final String accessToken) async {
    String clientID = dotenv.env['CLIENT_ID']==null ? '' : dotenv.env['CLIENT_ID']!;
  ...
  Future<String> _ocrDocument(final String assetID, final String accessToken) async {
    String base = dotenv.env['BASE']==null ? '' : dotenv.env['BASE']!;
  ...
  @override
  Future<Uint8List> ocrDocument(final pw.Document pdf, final Logger log) async {
    final String accessToken = await _retrieveAccessToken();
    final String assetID = await _uploadAsset(pdf, accessToken);
    final String location = await _ocrDocument(assetID, accessToken);
    return await _pollDocument(location, accessToken, log);
  }
}